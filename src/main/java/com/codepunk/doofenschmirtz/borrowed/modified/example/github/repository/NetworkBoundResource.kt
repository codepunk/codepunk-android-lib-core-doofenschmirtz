 ...
 *
 * Modifications:
 * August 2019: Use our own Resource class
 */

package com.codepunk.doofenschmirtz.borrowed.modified.example.github.repository
...
import com.codepunk.doofenschmirtz.borrowed.modified.example.github.api.ApiEmptyResponse
...
import com.codepunk.doofenschmirtz.borrowed.modified.example.github.vo.*
...
@MainThread constructor(private val appExecutors: AppExecutors) {
    ...
    init {
        result.value = PendingResource() /* TODO Resource.loading(null) */
        ...
        result.addSource(dbSource) { data ->
            ...
            if (shouldFetch(data)) {
                ...
                result.addSource(dbSource) { newData ->
                    setValue(SuccessResource(newData) /* TODO Resource.success(newData) */)
                }
            }
        }
    }
    ...
    private fun fetchFromNetwork(dbSource: LiveData<ResultType>) {
        ...
        result.addSource(dbSource) { newData ->
            setValue(LoadingResource(newData) /* TODO Resource.loading(newData) */)
        }
        result.addSource(apiResponse) { response ->
            ...
            when (response) {
                ...
                is ApiErrorResponse -> {
                    onFetchFailed()
                    result.addSource(dbSource) { newData ->
                        setValue(ErrorResource(newData, response.error) /* TODO Resource.error(response.errorMessage, newData) */)
                    }
                }
            }
        }
